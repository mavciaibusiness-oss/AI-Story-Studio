...
/*
  ---------- VİDEONUN SESİ VAR MI ----------

  Kullanıcı kararı: "videonun kendi sesi varsa ve kullanıcı sahne
  videolarının sesini kullanmak isterse seslendirmeye gerek
  kalmayacak."

  Bunu bilmek için sesin VARLIĞINI ölçmek gerekiyor.

  ---------------------------------------------------------------
  TARAYICI BUNU DOĞRUDAN SÖYLEMİYOR

  Standart bir API yok. Üç tarayıcıya özel alan var ve hiçbiri
  her yerde çalışmıyor:

    · webkitAudioDecodedByteCount  (Chrome, Safari)
    · mozHasAudio                  (Firefox)
    · audioTracks                  (kısmi destek)

  Hiçbiri yoksa `null` dönüyoruz — "ses yok" DEĞİL, "bilmiyoruz".
  Fark önemli: bilmediğimizde kullanıcıya soracağız, sessizce
  "sesi yok" varsaymayacağız.
  ---------------------------------------------------------------
*/
export function detectVideoAudio(file) {
  return new Promise((res) => {
    let url;
    try { url = URL.createObjectURL(file); } catch { return res(null); }
    ...
    v.onloadedmetadata = () => {
      /* audioTracks — en doğru olan, ama desteği kısmi */
      if (v.audioTracks && typeof v.audioTracks.length === 'number') {
        return done(v.audioTracks.length > 0);
      }
      /* Firefox */
      if (typeof v.mozHasAudio === 'boolean') return done(v.mozHasAudio);
      /*
        Chrome/Safari: çözülmüş ses baytı. Metadata anında 0
        olabiliyor — kısa bir oynatma denemesi gerekiyor.
      */
      if (typeof v.webkitAudioDecodedByteCount === 'number') {
        v.play().then(() => {
          setTimeout(() => {
            const has = v.webkitAudioDecodedByteCount > 0;
            try { v.pause(); } catch { /* önemsiz */ }
            done(has);
          }, 220);
        }).catch(() => done(null));
        return;
      }
      /* Hiçbir yol yok — BİLMİYORUZ */
      done(null);
    };
