...
/* Kategori eşlemesi TEK KAYNAKTAN. Adım 5'te buraya ikinci bir
   kopya yazmıştım; iki yerde farklı sayarsak Dashboard'ın "bu
   hafta" ve "bugüne kadar" bölümleri çelişirdi. */
import { categoryOf, CATEGORY_KEYS } from '@/lib/dashboard/summary';
...
export function lifetimeStats(episodes) {
  ...
  const byCategory = Object.fromEntries(CATEGORY_KEYS.map(k => [k, empty()]));
  let scenes = 0, withScenes = 0;
  let firstAt = null, lastAt = null;

  for (const p of eps) {
    const cat = categoryOf(p.format);
