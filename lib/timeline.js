...
/* Sahne süresi kuralları: görsel 5 sn · video kendi süresi (2026-08) */
import { sceneDuration } from './scene/duration';
...
export function buildTimeline(sb) {
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    /*
      ...
      Eskiden süre YALNIZCA seslendirmeden geliyordu: ses yoksa
      sahne "süresiz" sayılıyordu ve `videoDuration` alanı hiç
      okunmuyordu.

      Şimdi lib/scene/duration.js karar veriyor:
        · görsel sahne → 5 sn
        · video sahne  → videonun kendi süresi
        · uzun seslendirme varsa o kazanıyor

      Metin tahmini hâlâ devrede: ses üretilmemiş ama metin
      uzunsa 5 saniyeyi aşan süre ondan geliyor.
    */
    const est = words > 0 ? estimateSpokenDuration(text) : 0;
    const d = sceneDuration(s, { estimatedVoice: est });

    let dur = d.known ? d.seconds : 0;
    let source = d.source;
    let estimated = d.source === 'estimate' || d.source === 'image';
