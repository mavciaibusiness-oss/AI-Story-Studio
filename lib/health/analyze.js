...
/*
  ...
  AI katmanı (adım 2) bunun ÜSTÜNE biner: kurallar "sahne 7 çok uzun"
  ...
  Sahne süresi kaynağı: voiceDuration varsa o, yoksa metinden tahmin.
  Tahmin edilen sürelerle üretilen rapor `estimated: true` işaretlenir,
  arayüz bunu kullanıcıya söyler.
*/
...
/* Seslendirme yoksa süreyi metinden tahmin et.
   150 kelime/dk ortalama anlatım hızı + kısa sahnelere taban pay. */
function estimateDur(text) {
  const w = wordCount(text);
  if (!w) return 0;
  return Math.max(1.5, (w / 150) * 60);
}
...
function buildTimeline(scenes) {
  const out = [];
  let t = 0, estimated = false;
  for (let i = 0; i < scenes.length; i++) {
    const s = scenes[i];
    let dur = Number(s.voiceDuration) || 0;
    if (!dur) { dur = estimateDur(s.voiceText || s.paragraph); estimated = true; }
    out.push({
      scene: i + 1,
      at: +t.toFixed(2),
      end: +(t + dur).toFixed(2),
      dur: +dur.toFixed(2),
      words: wordCount(s.voiceText || s.paragraph),
      media: s.media === 'video' ? 'video' : 'image',
      hasMedia: !!(s.image || s.video),
      estimated: !Number(s.voiceDuration)
    });
    t += dur;
  }
  return { timeline: out, total: +t.toFixed(2), estimated };
}
...
export function analyzeStoryboard(sb) {
  ...
  const { timeline: tl, total, estimated } = buildTimeline(scenes);
