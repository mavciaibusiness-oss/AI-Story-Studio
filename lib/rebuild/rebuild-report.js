...
import { measureTechnical, missingDataCTAs, scoreBasis } from '@/lib/health/technical';
...
*/
export function filterForVideo(health, capabilities, technical) {
  ...
  /*
    ...
    Bunlar metne bağlı değil: video dosyasından geliyor. Senaryo
    olmasa da ölçülüyorlar ve kapsamı gerçekten genişletiyorlar.

    Kapsam %12 → ~%33 oluyor (technical 0.08 + platform 0.07 +
    audio 0.06 = 0.21 ek ağırlık).
  */
  for (const [k, v] of Object.entries(technical?.scores || {})) {
    if (Number.isFinite(v)) scores[k] = clampScore(v);
  }
  ...
  */
  /*
    ---------- EŞİK YENİDEN DÜŞÜNÜLDÜ ----------

    Eski eşik 0.50 mutlak ağırlıktı. Ama video dosyasından
    ölçülebilecek AZAMİ kapsam %38 (visual + pacing + technical +
    platform + audio = 0.46 / 1.21).

    Yani 0.50 eşiği dış videoya "asla puan yok" demek anlamına
    geliyordu — kullanıcının bildirdiği çıkmaz sokak tam olarak bu.

    Yeni ölçüt: KAÇ KATEGORİ ölçüldü.

    Dört ya da daha fazla bağımsız kategori ölçülmüşse ortada
    özetlenecek bir bütün var. Tek kategoriden (eski durum) puan
    çıkarmak hâlâ uydurma — o yüzden eşik kalktı değil, değişti.

    Ağırlık kontrolü de duruyor: 4 kategori ölçülse bile toplam
    ağırlık %20'nin altındaysa özet anlamsız.
  */
  /*
    EŞİK 3'E DÜŞTÜ.

    Sessiz bir video yüklendiğinde `audio` ölçülemiyor ve geriye
    3 kategori kalıyordu (ritim, teknik, platform) — ağırlıkça
    %27, eşiğin üstünde. Ama kategori sayısı 4'ün altında olduğu
    için puan yine verilmiyordu.

    Kullanıcı "yeterli ölçüm çıkmadı" görüyordu; oysa üç gerçek
    ölçüm vardı.

    AĞIRLIK KONTROLÜ DURUYOR: 3 kategori ölçülse bile toplam
    ağırlık %20'nin altındaysa özet anlamsız. Asıl koruma o.
  */
  const MIN_CATEGORIES_FOR_OVERALL = 3;
  const MIN_WEIGHT_FOR_OVERALL = 0.20;
  const enoughToSummarize = measured.length >= MIN_CATEGORIES_FOR_OVERALL
    && weightCovered >= MIN_WEIGHT_FOR_OVERALL;
  ...
  return {
    report: {
      ...
      /*
        ...
        söylüyor. Arayüz bunu "senaryonu ekle → içerik analizi"
        düğmesine çeviriyor.
      */
      needs: [
        ...(hasScript ? [] : [{
          data: 'script',
          unlocks: [...TEXT_CATEGORIES, ...SCRIPT_DEPENDENT_CATEGORIES]
        }]),
        ...((technical?.needs) || []).map(n => ({ data: n.need, unlocks: [n.cat] }))
      ],

      /* Platform: kullanıcı söylemediyse tahmin edildiği bildiriliyor */
      platform: technical?.platform || null,
      issues: kept,
...
*/
export function analyzeRebuild(scan, built, opts) {
  ...
  /*
    ...
    Senaryo yokken kapsam %12'de kalıyordu. Bu üç kategori video
    dosyasından ölçülüyor ve kapsamı gerçekten genişletiyor —
    puan uydurmadan.
  */
  const tech = measureTechnical(scan?.info, opts?.platform);
  const { report: health, suppressed } = filterForVideo(rawHealth, caps, tech);
  ...
  */
  ...
  /*
    ...
    Kullanıcı kararı: "eksik veri bir HATA değil, kullanıcıyı bir
    sonraki adıma götüren bir CTA olmalı".

    `locked` listesi zaten "neler kapalı" diyordu ama pasifti.
    `ctas` aynı bilgiyi eyleme çeviriyor: ne verirsen ne açılır,
    nereye gideceksin.
  */
  const ctas = missingDataCTAs({
    scores: health?.scores || {},
    info: scan?.info,
    target: opts?.platform
  });

  /* Puan neye dayanıyor — kullanıcı sağlamlığını görsün */
  const basis = scoreBasis(health?.scores || {}, HEALTH_CATEGORIES);
