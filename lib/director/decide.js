...
*/
/* opts.weights — TASK-03 geri bildirim ağırlıkları (isteğe bağlı) */
export function directProject(sb, opts) {
  const scenes = Array.isArray(sb?.scenes) ? sb.scenes : [];
  ...
  /* Kullanıcı ağırlıkları (TASK-03): geçmiş geri bildirimden gelir.
     Verilmezse nötr — hafıza kapalıyken davranış değişmez. */
  const sorted = sortRecommendations(recs, opts?.weights);
