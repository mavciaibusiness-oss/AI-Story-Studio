...
/* TASK-03: kullanıcının geri bildirim geçmişi öneri sırasını etkiliyor */
import { feedbackWeights } from '@/lib/creator/personalize';
...
/*
  Kullanıcının öneri ağırlıkları — Creator Memory'den.

  Hafıza yoksa (migration v10 uygulanmamış) boş dönüyor ve Director
  eskisi gibi çalışıyor. Öğrenme isteğe bağlı bir katman; olmadığında
  ürün bozulmamalı.
*/
async function userWeights(supabase, userId) {
  try {
    const { data, error } = await supabase
      .from('creator_memory').select('memory')
      .eq('user_id', userId).maybeSingle();
    if (error || !data?.memory) return null;
    return feedbackWeights(data.memory);
  } catch {
    return null;
  }
}
...
export async function POST(req) {
  try {
    ...
    /* Öneri ağırlıkları (TASK-03): kullanıcının geri bildirim
       geçmişinden. Bir kez okunuyor, tüm eylemler paylaşıyor.
       Hafıza yoksa null — Director eskisi gibi çalışır. */
    const weights = await userWeights(supabase, user.id);
    ...
    if (action === 'direct') {
      const director = directProject(sb, { weights });
    ...
    if (action === 'apply') {
      ...
      const after = directProject(nextSb, { weights });
    ...
    if (action === 'applyMany') {
      ...
      for (const id of ids) {
        const director = directProject(currentSb, { weights });
        const rec = director.recommendations.find(r => r.id === id);
      ...
      if (!applied.length) {
        return NextResponse.json({
          ...
          director: directProject(sb, { weights })
        }, { status: 400 });
      }
      ...
      const afterMany = directProject(currentSb, { weights });
