...
/*
  ...
  `userWeights` (Sprint 5 / TASK-03): kullanıcının geri bildirim
  geçmişinden gelen ağırlıklar. Sürekli reddettiği öneri türü geriye
  alınıyor, kabul ettiği öne çıkıyor.

  ÖNERİ GİZLENMİYOR — yalnızca sıralaması değişiyor. Ağırlık 0.6'nın
  altına inmiyor (lib/creator/personalize.js). Gizlemek kullanıcının
  fikrini değiştirme ihtimalini yok sayardı.

  Ağırlık verilmezse 1 (nötr) — hafıza kapalıyken davranış aynı kalır.
*/
export function sortRecommendations(recs, userWeights) {
  const score = (r) => {
    const pts = r.impact?.points || 1;
    const staticW = KIND_WEIGHT[r.kind] || 1;
    const userW = Number(userWeights?.[r.kind]) || 1;
    return pts * r.confidence * staticW * userW;
  };
