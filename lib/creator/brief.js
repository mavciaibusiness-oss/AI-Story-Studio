...
export function buildBrief(args) {
  /*
    NULL KORUMASI — Adım 5'te yakalandı.

    Adım 2'de yazdığım "null çökmez" testi yanlış kurulmuştu:
    `buildBrief(null)` gerçekten TypeError atıyordu. Çağıran taraf
    (CreatorView) aktif oturum yoksa null geçebilir.
  */
  const { classified, workflow, memory, timings } = args || {};
