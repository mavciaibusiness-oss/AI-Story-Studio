...
export function normalizeBrand(input) {
  return {
    ...
    createdAt: input?.createdAt || new Date().toISOString(),
    /*
      Marka NE ZAMAN güncellendi — Creator Intelligence Adım 6.

      Memory health bunu kullanıyor: marka güncellemesinden ÖNCE
      üretilmiş projeler o tonla üretilmemiş demektir. Alan yoksa
      sürüklenme tespit edilemezdi.

      Yeni kayıtta createdAt ile aynı; güncellemede updateBrand
      tazeliyor.
    */
...
export function updateBrand(memory, brandId, patch) {
  ...
  const next = [...m.brands];
  /* updatedAt TAZELENİYOR — patch'ten gelen değer yok sayılıyor.
     İstemcinin geçmiş bir tarih göndermesi sürüklenme tespitini
     bozardı. */
  next[i] = normalizeBrand({
    ...next[i], ...patch, id: brandId,
