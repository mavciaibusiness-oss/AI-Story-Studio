...
/*
  ...
  Öneri gerekçesinin i18n anahtarı. İlk sürümde CreatorView içindeydi
  ve WorkspaceParts oradan import ediyordu — döngüsel bağımlılık.
  Gerekçe zaten bu dosyanın ürettiği bir şey; buraya ait.

  Metin YOK: anahtar ve veri dönüyor, arayüz t() ile kuruyor.
*/
export function reasonKey(suggestion) {
  switch (suggestion?.reason) {
    case 'stale':    return { key: 'cos.why.stale', vars: {} };
    case 'continue': return { key: 'cos.why.continue', vars: {} };
    case 'unlocks':  return { key: 'cos.why.unlocks',
                              vars: { n: suggestion.detail?.unlocks ?? 0 } };
    default:         return { key: 'cos.why.next', vars: {} };
  }
}
