...
export function learn(memory, sources) {
  ...
  for (const s of (src.sessions || [])) {
    ...
    if (o.workflow.samples > 0) observations.push(o);
    newSessions.push(id);
  }
...
export function buildProposals(memory) {
  ...
  /* Dil: içerik dilinden çıkarılamaz (storyboard.language kaydediliyor
     ama gözleme almadık — bilinçli, ileride eklenebilir). Şimdilik
     dil önerisi üretmiyoruz; uydurmaktansa susmak. */
