...
/*
  ...
  Sprint 6 / TASK-05, Adım 3.

  Kullanıcı ne kadar süre sonra döndü? Karşılamanın tonu buna göre
  değişiyor.

  SUÇLULUK YOK (kullanıcının kuralı): "3 haftadır yoksun" demiyoruz.
  Sadece bağlam kuruyoruz — uzun aradan sonra dönen kullanıcının
  nerede kaldığını hatırlaması gerekiyor, azarlanması değil.
*/
export const GAP_BANDS = [
  { key: 'same-day', maxDays: 1 },
  { key: 'recent',   maxDays: 3 },
  { key: 'away',     maxDays: 14 },
  { key: 'long',     maxDays: Infinity }
];

export function returnGap(since, now) {
  if (!since) return { known: false, band: 'same-day' };
  const t = new Date(since).getTime();
  if (!Number.isFinite(t)) return { known: false, band: 'same-day' };

  const days = Math.floor(((now ?? Date.now()) - t) / 86400000);
  const band = GAP_BANDS.find(b => days < b.maxDays)?.key || 'long';
  return { known: true, days, band };
}

/*
  ---------- AKTİF PLANDA GÜNLÜK BAĞLAM ----------

  Adım 2'de karşılama YALNIZCA boş durumda görünüyordu. Plan
  üzerinde çalışan kullanıcı günlük bağlamı hiç görmüyordu.

  Ama aktif planda ekran zaten dolu: yol haritası, plan özeti,
  görev listesi. Oraya bir karşılama daha koymak gürültü olur.

  ÇÖZÜM: tek satırlık bağlam. "Bugün 3. günün" ya da "5 gün sonra
  döndün, buradaydın." Durum çubuğunun yanında, küçük.

  Çıkış null olabilir — söylenecek bir şey yoksa satır hiç
  görünmüyor.
*/
export function activeContext({ daily, now }) {
  if (!daily) return null;
  const gap = returnGap(daily.since?.since, now);

  /*
    ÖNCELİK: dönüş aralığı > seri > sessizlik

    Uzun aradan dönen kullanıcı için "12 gündür üretiyorsun" demek
    tuhaf olur — o seri zaten kırılmış. Önce dönüşü karşılıyoruz.
  */
  if (gap.known && (gap.band === 'away' || gap.band === 'long')) {
    return { kind: 'welcome-back', days: gap.days, band: gap.band };
  }

  /* Seri varsa ve bugün henüz çalışılmadıysa: devam daveti */
  if (daily.streak?.active && !daily.streak.includesToday) {
    return { kind: 'continue-streak', days: daily.streak.current };
  }

  /* Bugün çalışılmış ve seri varsa: sessiz onay */
  if (daily.streak?.active && daily.streak.includesToday && daily.streak.current > 1) {
    return { kind: 'streak-today', days: daily.streak.current };
  }
