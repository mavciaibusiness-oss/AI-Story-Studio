...
*/
export function buildNotifications({ sessions, active, memory, dismissed }) {
  const out = [];
  ...
  if (memory) {
    ...
    /* Hafıza yeni bir şey öğrendiyse — onay bekleyen çıkarım */
    const props = memory.__proposals;   // API'den geliyorsa
    if (Array.isArray(props) && props.length) {
      out.push(n('mem:proposals:' + props.map(p => p.field).join(','),
        'action', 'memory-proposals', { count: props.length }));
    }
  }
