...
export const WIDGETS = {
  ...
  /*
    ...
    'recent' panosu Creator OS OTURUMLARINI gösteriyor — yani
    Workspace üzerinden başlatılmış planları. Ama kullanıcı
    modüllerden doğrudan da video üretebiliyor ve o işler Workspace'te
    hiç görünmüyordu.

    Bu pano `projects` (episodes tablosu) verisini gösteriyor:
    yarım kalanlar ve durumları. Spec: "Project Manager Workspace'in
    ana veri kaynağıdır."
  */
  projects: {
    key: 'projects', needs: 'projects', weight: 85,
    label: { tr: 'Videoların', en: 'Your videos' }
  }
};
...
function dynamicWeight(key, ctx) {
  ...
  /* Yarım kalan iş varsa videolar panosu öne çıkıyor — kullanıcının
     en çok ihtiyaç duyduğu bilgi o. */
  if (key === 'projects' && (ctx.unfinished || []).length > 0) return w + 20;
...
export function widgetData(key, ctx) {
  ...
  switch (key) {
    ...
    case 'projects': {
      ...
      /* Yarım kalanlar önce — asıl aksiyon gerektiren onlar.
         Sonra en son güncellenenler. */
      const shown = [
        ...unfinished.slice(0, 3),
        ...all.filter(p => !unfinished.some(u => u.id === p.id)).slice(0, 2)
      ].slice(0, 4);
      return {
        items: shown.map(p => ({
          id: p.id, title: p.title, status: p.status,
          idleDays: p.idleDays ?? null,
          ready: p.ready?.total ? Math.round((p.ready.media / p.ready.total) * 100) : null
        })),
        total: all.length,
