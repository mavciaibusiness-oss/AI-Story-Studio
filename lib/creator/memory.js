...
*/
export const CRITICAL = ['creatorType', 'level', 'primaryGoal'];

/*
  DİL BURADA TUTULMUYOR — TEK KAYNAK profiles.settings.prodLang

  İlk yazışta `profile.language` alanı vardı. Ama üretim dili zaten
  `profiles.settings.prodLang`'da ve uygulama ONU kullanıyor: storyboard
  kuran 11 çağrı noktası oradan okuyor, Ayarlar sayfası orayı yazıyor.

  İki yerde tutmak spec'in uyardığı şeydi ("Gereksiz tekrar
  oluşturma") ve pratikte iki değer birbirinden ayrışırdı: kullanıcı
  Ayarlar'dan değiştirse hafıza eski değeri gösterirdi.

  Hafıza ekranı dili GÖSTERİYOR ama profiles.settings'ten okuyor ve
  değiştirmek için Ayarlar'a yönlendiriyor.
*/
...
export function emptyMemory(patch) {
  return {
    ...
    /*
      NİYETLER — Sprint-6 TASK-02, Adım 5.

      Kullanıcının NE İSTEDİĞİ. `content.genres` üretilen storyboard'dan
      öğreniliyor; bu ondan farklı: kullanıcının kendi ifadesi.

      Fark önemli. Biri "korku videosu" isteyip sonuçta gerilim
      üretebilir; ikisi de gerçek ama farklı sinyaller. Niyet
      "ne istiyor", tür "ne çıkıyor".

      Anahtar olarak niyet KEY'i saklanıyor (video.horror) — metin
      değil. TASK-03'ün gizlilik kuralı: kullanıcı cümlesi asla
      hafızaya girmez.
    */
...
export function mergeObservations(memory, obs) {
  ...
  return {
    ...
    /* Sprint-6 TASK-02: niyetler */
    intents: {
      keys:    mergeCounts(m.intents?.keys, obs.intents?.keys),
      samples: (m.intents?.samples || 0) + (obs.intents?.samples || 0)
    },
    style: {
