...
/* ---------- Yardımcı ----------
   Bir prompt/metinde bir terim listesi kaç kez geçiyor?

   KELİME BAŞI SINIRI — neden gerekli:
     Saf alt dizi taraması sessiz yanlış eşleşme üretir. "ev" terimi
     "cevap", "devam", "sevgi" içinde bulunur ve motor mekân var sanır.
     Bu tür hata ölçümü güvenilmez kılar çünkü kullanıcı sebebini
     göremez.

   NEDEN SAF KELİME SINIRI DA OLMAZ:
     Türkçe sondan eklemeli. "ormanda", "şehirden", "kalenin" hepsi
     meşru eşleşmedir; kelime sonuna sınır koyarsak bunları kaçırırız.

   ÇÖZÜM: terim KELİME BAŞINDA başlamalı, sonuna ek alabilir.
     "ormanda"  → "orman" kelime başında  ✓
     "cevap"    → "ev" harften sonra      ✗
     "wide shot"→ çok kelimeli ibare      ✓ (aynı kural ilk kelimeye)
*/
export function countTermHits(text, terms) {
  ...
  for (const term of terms) {
    ...
    if (matchesAtWordStart(lower, t)) { hits.push(term); seen.add(t); }
  }
  return { count: hits.length, hits };
}

/* Terim metinde bir kelime başında geçiyor mu?
   Regex kurmak yerine indexOf döngüsü: terimler kullanıcı verisi değil
   ama içlerinde regex özel karakteri olabilir (nokta, tire, parantez).
   Kaçırma derdine girmeden doğrudan konum kontrolü yapıyoruz. */
function matchesAtWordStart(haystack, needle) {
  ...
  while (true) {
    ...
    if (i === -1) return false;
    // Başta mı, yoksa öncesinde harf/rakam olmayan bir şey mi var?
    const prev = i === 0 ? '' : haystack[i - 1];
    if (i === 0 || !isWordChar(prev)) return true;
    from = i + 1;
  }
}

/* Harf ya da rakam mı? Türkçe harfler dahil.
   \p{L} Unicode harf sınıfı; ı, ş, ğ, ü, ö, ç hepsini kapsar. */
